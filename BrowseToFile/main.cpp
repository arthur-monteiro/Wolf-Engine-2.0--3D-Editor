...
void BrowseToFile(std::string& filename, BrowseToFileOption option, BrowseToFileFilter filter)
{
    ...
    {
        std::cerr << "Unable to initialize GTK.\n";
        return;
    }
    ...
    case BrowseToFileFilter::IMG:
        ...
        break;
    case BrowseToFileFilter::SCRIPT:
        gtk_file_filter_set_name(gtk_filter, "Script");
        gtk_file_filter_add_pattern(gtk_filter, "*.lua");
        break;
    }
...
int main(int argc, char** argv)
{
    if (argc < 4)
    {
        std::cout << "Usage: " << argv[0] << " <open|save> <filter_type> <folder_restriction>\n";
        return EXIT_FAILURE;
    }
    ...
    while (filename.empty() || filename.substr(0, folderRestriction.size()) != folderRestriction)
    {
        ...
        if (filename.empty())
        {
            std::cout << "Action cancelled by user.\n";
            return EXIT_SUCCESS;
        }
    }
